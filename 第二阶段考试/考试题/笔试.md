...
6. 以下哪个语句打印出来的结果是 true ：( ) 
   (A) alert("12" === 12);
   (B) alert(NaN === NaN);
   (C) alert(typeof(null) === typeof(window));   返回的是字符串
   (D) alert([1,2,3] === [1,2,3]);
...
8. 有一个 HTML  页面，其源码中有如下内容( ) 
   ...
   (C) document.getElementById('myform').childNodes[1].value 文本算
...
17. 以下描述中错误的一个是：
    ...
    (D) 在 JavaScript 中如果访问数组时越界会导致浏览器报错   没有越界概念
