...
const actions = {
  // 用于初始化的时候从数据库获取数据
  getCart ({commit}) {
    http.get('http://10.3.157.225:8081/cart')
      .then(function (data) {
  ...
  addCart ({commit}, item) {
    ...
    http.get('http://10.3.157.225:8081/cart?shop_id=' + item.shop_id)
      .then(function (data) {
        if (data.data.length > 0) {
          ...
          http.put('http://10.3.157.225:8081/cart/' + obj.id, newObj)
            .then(function (data) {
          ...
          // 添加用post
          http.post('http://10.3.157.225:8081/cart', obj)
            .then(function (data) {
              commit('ADD_CART', data.data)
            })
        }
  ...
  decrease ({commit}, item) {
    ...
    http.put('http://10.3.157.225:8081/cart/' + item.id, obj)
      .then(function (data) {
        // console.log(data.data)
  ...
  del ({commit}, item) {
    http.delete('http://10.3.157.225:8081/cart/' + item.id)
      .then(function () {
        // console.log(data.data)
        commit('DEL', item)
      })
